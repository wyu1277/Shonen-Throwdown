...
const Loading = () => {
	...
	useEffect(() => {
		...
		channel
			.on('presence', { event: 'join' }, ({ key, newPresences }) => {
				...
				channel.on('broadcast', { event: 'getUserDeck/' + Router.query.id }, (payload) => {
					console.log(payload, 'LOADING PAYLOAD BROADCAST');
					dispatch(gameActions.setPlayer2Deck(payload.payload.data?.userDeck));
					dispatch(gameActions.setPlayer2(payload.payload.data?.player));
				});
	...
	useEffect(() => {
		if (player2) {
			const setPlayer2 = async () => {
				await supabase.from('game').update({ player2: player2.id }).eq('id', Router.query.id);
			};
			setPlayer2();
		}
	}, [player2]);
	...
	return <div>{localLoading ? <div>LOADING...</div> : <button onClick={playGame}>Click to Play!</button>}</div>;
};
