...
const Collection = () => {
  ...
  if (user) {
    const filteredData =
      data &&
      data.filter((card) =>
        card.cards.name.toLowerCase().includes(searchInput.toLowerCase())
      );
    ...
    if (filteredData) {
      ...
      if (sortValue === "name") {
        const nameSort = sortedData.sort((a, b) =>
        ...
        const powerSort = sortedData.sort(
          (a, b) => b.cards.power - a.cards.power
        );
        powerSort.sort((a, b) => {
          if (a.quantity > 0 && b.quantity <= 0) {
            return -1;
        ...
        const elementSort = sortedData.sort((a, b) =>
          a.cards.element.localeCompare(b.cards.element)
        );
        ...
        const seriesSort = sortedData.sort((a, b) =>
          a.cards.series.localeCompare(b.cards.series)
        );
        seriesSort.sort((a, b) => {
          if (a.quantity > 0 && b.quantity <= 0) {
            return -1;
